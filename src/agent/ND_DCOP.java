 ...
 *
 */
public class ND_DCOP extends Agent implements DCOP_INFO {
	...
	private ArrayList<String> bestImproveValueListJESP;
	private ArrayList<Double> bestUtilityList;
	private Double bestImproveUtilityJESP;
	...
	private double oldLSRunningTime = 0; // old running time because compare to
											// see if old iteration is converged
	...
	public void readArguments() {
		...
		varDecisionFileName = ND_DCOP.algTypes[algorithm] + "_d=" + noAgent + "_sw=" + (int) switchingCost + "_h=" + h
				+ ".txt";
...
//			}
//		}
		// create distribution at each time step, and stationary distribution at
		// h
//		if (algorithm == FORWARD || algorithm == BACKWARD || algorithm == LS_SDPOP || algorithm == LS_RAND) {
		if (algorithm == JESP) {
...
//		}
		...
		// correct
//		if (algorithm == FORWARD || algorithm == BACKWARD || algorithm == LS_SDPOP || algorithm == REACT
//				|| algorithm == HYBRID) {
//			for (int i = 0; i <= h; i++) {
//				mainSequentialBehaviourList.addSubBehaviour(new DPOP_UTIL(this));
//				mainSequentialBehaviourList.addSubBehaviour(new DPOP_VALUE(this));
...
//		if (algorithm == LS_SDPOP || algorithm == LS_RAND) {
	...
	protected void takeDown() {
		...
		System.out.println("Agent " + idStr + " with threadID " + Thread.currentThread().getId() + " has sim TIME: "
	...
	public void initializeArguments() {
		...
		bestImproveValueListJESP = new ArrayList<String>();
		bestUtilityList = new ArrayList<Double>();
//		bestImproveUtilityJESP = new Double();
	...
	public void sendImprove() {
		...
		for (int index = 0; index < totalSize; index++) {
			...
			if (evaluation > maxUtility) {
				...
				bestImproveValueListJESP = valueListTS;
			}
		}

//		bestImproveUtilityJESP = new ArrayList<Double>();
		bestImproveUtilityJESP = null;
		
		if (maxUtility != Integer.MIN_VALUE) {
//			bestUtilityList = utilityMinusCostOverTSList(bestImproveValueList);
//			// if (idStr.equals("1")) System.err.println("best " +
//			// bestUtilityList);
//			for (int i = 0; i <= h; i++) {
//				bestImproveUtilityJESP.add(bestUtilityList.get(i) - currentUtilityList.get(i));
//			}
			bestImproveUtilityJESP = new Double(maxUtility);
		}
		...
		for (AID neighbor : neighborAIDList) {
			sendObjectMessageWithTime(neighbor, bestImproveUtilityJESP, MESSAGE_TYPE.LS_IMPROVE, simulatedTime);
		}
	}
	...
	ArrayList<Double> utilityMinusCostOverTS(HashMap<Integer, String> valuesOverTS) {
		...
		for (int ts = 0; ts <= h; ts++) {
			...
			if (ts == 0)
				...
				// sc = (valuesOverTS.get(ts-1).equals(valuesOverTS.get(ts)) ? 0
				// : switchingCost)
				// + (valuesOverTS.get(ts).equals(valuesOverTS.get(ts+1)) ? 0 :
				// switchingCost);
	...
	public void createStableProbabilityForLastTimeStep() {
		...
		for (String randVariable : randomVariableList) {
			...
			for (int i = 0; i <= ND_DCOP.stableTimeStep; i++) {
...
//			}
	...
	public Table joinConstraintTable(ArrayList<Table> tableList) {
		...
		for (int count = 0; count < totalSize; count++) {
			...
			double sumUtility = 0;
			int quotient = count;
			// for each table count, decide the index of each column, then add
			// to the tuple
		...
		for (Table randTable : organizedConstraintTableWithRandomList) {
			...
			for (int tS = 0; tS <= h; tS++) {
				...
				for (int index = 0; index < tempTable.getRowCount(); index++) {
					...
					for (int idx = 0; idx < randLabel.size(); idx++) {
						String rand = randLabel.get(idx);
						initProb *= probabilityAtEachTimeStepMap.get(rand)[tS][randomVariableDomainMap.get(rand)
								.indexOf(randValueList.get(idx))];
					}
					expectedUtility += initProb * row.getUtility();
...
//			}
	...
	// ArrayList<ArrayList<String>> processedDecValues = new
	...
	// ArrayList<ArrayList<String>> allTupleValue =
	// getAllTupleValueOfGivenLabel(randVarLabel, false);
	...
	// else if (isArrayContainedInOtherArray(processedDecValues,decValueList) ==
	// false) {
	...
	// }
	// //set utility
	// coefficients[row][noOfEquations] =
	// getUtilityFromTableGivenDecAndRand(tableWithRandom, decValueList,
	// rowTuple);
	// }
	//
	...
	// Row newRow = new Row(decValueList, randValueToBeAddedList,
	// utilityList.get(i));
	// i++;
	...
	double getUtilityFromTableGivenDecAndRand(Table table, ArrayList<String> decValueList,
			ArrayList<String> randIterationValue) {
		...
		try (BufferedReader br = new BufferedReader(
			...
			for (String lineWithSemiColon : lineWithSemiColonList) {
				...
				if (nameMzn.contains(TRANS_FUNC_PREFIX)) {
					...
					if (randomVariable.equals(idStr) == false)
						continue;
				...
				if (nameMzn.contains(REWARD_TABLE_PREFIX)) {
					...
					/**** WITH random variable **********/
						...
						listOfRow = new ArrayList<String>(Arrays.asList(valueMzn.split("\\|")));
						for (String oneRow : listOfRow) {
							ArrayList<String> decAndRandValueList = new ArrayList<String>(
	...
	public double calculcatingSwitchingCost() {
		double sC = 0;
		if (getValueAtEachTSMap().size() == 1)
			return 0;
		for (int i = 1; i < getValueAtEachTSMap().size(); i++) {
			// if
			// (getValueAtEachTSMap().get(i).equals(getValueAtEachTSMap().get(i-1))
			// == false)
			// sC += switchingCost;
	...
	public void setDecisionVariableDomainMap(HashMap<String, ArrayList<String>> decisionVariableDomainMap) {
	...
	public void setConstraintTableAtEachTSMap(HashMap<Integer, ArrayList<Table>> constraintTableAtEachTSMap) {
	...
	public void setCurrentGlobalUtilityList(ArrayList<Double> currentGlobalUtilityList) {
	...
	public static void setDelayMessageTime(long delayMessageTime) {
		ND_DCOP.delayMessageTime = delayMessageTime;
	}
	...
	public void setAgentView_DPOP_TSMap(HashMap<String, HashMap<Integer, String>> agentView_DPOP_TSMap) {
	...
	public Double getBestImproveUtilityJESP() {
		return bestImproveUtilityJESP;
	}

	public void setBestImproveUtilityJESP(Double bestImproveUtilityList) {
		this.bestImproveUtilityJESP = bestImproveUtilityList;
	}

	public ArrayList<String> getBestImproveValueListJESP() {
		return bestImproveValueListJESP;
	}

	public void setBestImproveValueListJESP(ArrayList<String> bestImproveValueList) {
		this.bestImproveValueListJESP = bestImproveValueList;
	}
