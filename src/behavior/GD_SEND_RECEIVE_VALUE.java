...
public class GD_SEND_RECEIVE_VALUE extends OneShotBehaviour implements MESSAGE_TYPE {
  ...
  public void action() {
    ...
    for (AID neighborAgentAID : agent.getNeighborAIDSet()) {
      agent.sendObjectMessageWithTime(neighborAgentAID, agent.getChosenDoubleValueAtEachTSMap(),
          GD_VALUE, agent.getSimulatedTime());
    }
    
    List<ACLMessage> receivedMessageFromNeighborList = waitingForMessageFromNeighborWithTime(GD_VALUE);
